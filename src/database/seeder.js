...
// Minimum relatedness for a pair to be stored. Score is computed on
// tone-stripped base forms (see DainameseNormalizer.relatedScore): 1.0 = same
// base form (tone variants), ~0.8 = one vowel-quality change, ~0.67 = one
// consonant change (e.g. đ↔d). 0.55 keeps same-base words and up to ~2 light
// vowel edits, while dropping distant fold-siblings.
const MIN_RELATED_SCORE = 0.55;
// Candidate gate: fold-level edit distance (plain Levenshtein on the
// legacy đ→d + diacritic-stripped form) used only to bound the candidate
// pool cheaply before scoring base forms.
const MAX_FOLD_DIST = 3;
...
class DatabaseSeeder {
  ...
  processEntry(entry) {
    return {
      ...
      normalizedWord:
        ...
        this.normalizer.normalizeForSearch(entry.word || ""),
  ...
  async generateRelatedWords(db) {
    try {
      ...
      const rows = await this.allQuery(
        db,
        "SELECT id, word, normalized_word FROM words",
      );
      ...
      await logger.error("Related words generation failed", {
        error: error.message,
      });
      throw error;
    }
  ...
  computeRelated(target, normMap, buckets, byId) {
    ...
    const baseTarget = this.normalizer.normalizeBaseWord(target.word || "");
    ...
    for (const id of candidateIds) {
      ...
      if (!other) continue;
      const fold = other.normalized_word || "";
      const foldDist = levenshtein(norm, fold);
      const isSameFold = fold === norm;
      const isPrefix = fold.startsWith(norm) && fold.length > norm.length;
      // Cheap candidate gate (fold level) before Vietnamese-aware scoring.
      if (!isSameFold && foldDist > MAX_FOLD_DIST && !isPrefix) continue;
      const score = this.normalizer.relatedScore(
        baseTarget,
        this.normalizer.normalizeBaseWord(other.word || ""),
      );
      if (score < MIN_RELATED_SCORE) continue;
      scored.push({ id: other.id, word: other.word, score, dist: foldDist });
    }
    scored.sort((a, b) => {
      if (b.score !== a.score) return b.score - a.score;
      if (a.dist !== b.dist) return a.dist - b.dist;
      // Deterministic Vietnamese-aware tie-break (VNI tone order).
      return this.normalizer.vietnameseCompare(a.word, b.word);
    });
  ...
  async updateSearchIndex(db) {
    try {
      ...
      await logger.error("Search index update failed", {
        error: error.message,
      });
      throw error;
    }
  ...
  async logSeedingOperation(db, totalEntries, successCount, errorCount) {
    try {
      ...
      await logger.error("Failed to log seeding operation", {
        error: error.message,
      });
    }
  }
  ...
  async getSeedingStats(db) {
    try {
      const wordCount = await this.getQuery(
        db,
        "SELECT COUNT(*) as count FROM words",
      );
      const compoundCount = await this.getQuery(
        db,
        "SELECT COUNT(*) as count FROM compounds",
      );
      const relatedCount = await this.getQuery(
        db,
        "SELECT COUNT(*) as count FROM related_words",
      );
      const avgQuality = await this.getQuery(
        db,
        "SELECT AVG(text_quality) as avg_quality FROM words",
      );
      return {
      ...
      await logger.error("Failed to get seeding stats", {
        error: error.message,
      });
      return {
        totalWords: 0,
