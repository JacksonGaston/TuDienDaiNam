...
class DatabaseGenerator {
  constructor() {
    this.dbPath = path.join(
      __dirname,
      "../../frontend/assets/database/dictionary.db",
    );
  ...
  async initialize() {
    try {
      ...
      await logger.error("Database generator initialization failed", {
        error: error.message,
      });
      throw error;
    }
  ...
  async generateDatabase(options = {}) {
    try {
      ...
      if (textFiles.length === 0) {
        throw new Error(
          "No text files found in /data directory. Please add dictionary text files first.",
        );
      }

      const parsedData = await this.parser.batchParse(textFiles);
      await logger.info(
        `Parsed ${parsedData.entries.length} dictionary entries`,
      );

      const validationResults = this.validator.batchValidate(
        parsedData.entries,
      );
      const validEntries = this.validator.filterValidEntries(
        validationResults.results,
        options.minConfidence || 0.5,
      );

      await logger.info(
        `Validated entries: ${validEntries.length}/${parsedData.entries.length} valid`,
      );
      ...
      await logger.error("Database generation failed", {
        error: error.message,
      });
      throw error;
    }
    ...
    try {
      ...
      for (const file of textFiles) {
        try {
          ...
          await logger.warn(`Failed to load text file: ${file.name}`, {
            error: error.message,
          });
        }
      }
  ...
  async addIdsToEntries(entries) {
    try {
      ...
      for (const entry of entries) {
        const row = await this.getQuery("SELECT id FROM words WHERE word = ?", [
          entry.word,
        ]);
        if (row) {
      ...
      await logger.error("Failed to add IDs to entries", {
        error: error.message,
      });
      throw error;
    }
  ...
  async clearExistingData() {
    try {
      ...
      await logger.error("Failed to clear existing data", {
        error: error.message,
      });
      throw error;
    }
  ...
  async verifyDatabase() {
    try {
      ...
      await logger.error("Database verification failed", {
        error: error.message,
      });
      throw error;
    }
  ...
  async logGenerationComplete(stats, seedingResult) {
    try {
      await this.schema.logOperation(
        this.db,
        "database_generation",
        "success",
      ...
      await logger.error("Failed to log generation completion", {
        error: error.message,
      });
    }
  }
...
async function main() {
  ...
  try {
    ...
    console.log(
      `Average confidence: ${(result.stats.averageConfidence || 0).toFixed(2)}%`,
    );
    console.log(
      `Success rate: ${result.seedingResult.successRate.toFixed(2)}%`,
    );
